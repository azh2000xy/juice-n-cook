...
| EPUB 果蔬汁断食法 | 120 | ~150 | ~80% | 子串匹配去重 24 道 |
...
### D. 部署更新
```
git push → Render（通常自动部署，约2分钟）
如未自动触发 → dashboard.render.com → juice-n-cook → Manual Deploy → Deploy latest commit
新用户直接访问即可；老用户如见旧版 → F12 → Application → Service Workers → Unregister → 硬刷新
```

## 核心设计决策

1. **统一YAML frontmatter**：全部6,029道菜谱统一使用YAML frontmatter + Markdown格式
2. **食材子串匹配**：用户搜"鲍鱼"→命中"净鲜鲍鱼250克""水发鲍鱼"等带量食材名（2026-06 从精确集合交集改为子串匹配，修复90%+食材匹配失败问题）
3. **双模式搜索**：🎯尽量少补食材（score=匹配/总食材） / 🔍包括该食材即可（有交集即列入），前端 toggle 切换
4. **max_results 30**：从 5 提升至 30，单食材搜索不再被截断
5. **食材别名扩展**：用户输入"咸猪蹄"→匹配索引中的"猪蹄"（通过ingredients_index.yaml别名）
6. **步骤预计算**：build_index.py提取前3步存入索引，搜索API不读文件
7. **双栖食材**：番茄/黄瓜/胡萝卜等标记为fruit-vegetable，同时推荐菜谱+果蔬汁
8. **多数据源**：从YAML frontmatter的`source`字段读取来源标签，build_index.py自动统计
9. **分类先去重**：导入新食谱时先自动推断品类，仅与同品类现有菜谱比对（效率10-50x提升，消除跨品类误判）
...
4. **PWA 缓存旧UI**：SW cache-first 策略会缓存HTML。修复：v2改用 network-first（HTML）+ cache-first（静态资源），升级版本号触发更新。用户如仍见旧版→`F12 → Application → Service Workers → Unregister`
5. **Render 部署不一定自动触发**：git push 后如未自动部署→`dashboard.render.com → juice-n-cook → Manual Deploy → Deploy latest commit`
6. **食材名含量词**：子串匹配已缓解，但食材显示仍带量词（如"鲍鱼一只250克"），后续可做纯净食材名提取

## 下一步可做的

- [ ] 从带量食材中提取纯净食材名（如"海参一只250克"→"海参"），索引存两份（原始+纯净）
- [ ] 提高大PDF OCR质量（200+ DPI重跑，或用PaddleOCR）
- [ ] 加营养数据字段（卡路里/蛋白质）
- [ ] 用户反馈机制（"做过""好吃""太麻烦"）
- [ ] 购物清单自动汇总
- [ ] 自动分类精度提升（EPUB 来自 28 个菜系的精确标签可辅助分类）
- [ ] 果蔬汁仍有 ~30 道 EPUB 配方未提取（解析遗漏），可回扫补齐
